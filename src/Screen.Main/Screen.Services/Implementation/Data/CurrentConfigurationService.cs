...
public class CurrentConfigurationService : ICurrentConfigurationService
{
	...
	public CurrentConfigurationService(IConfigurationRepository configurationRepository, IMarqueeRepository marqueeRepository, IPageRepository pageRepository)
	{
	...
	public void Reload()
	{
		...
		CurrentConfiguration.Pages =
			_pageRepository.GetAllCustom(x => x.ConfigurationId == CurrentConfiguration.Id).OrderBy(x => x.Position).ToList();
