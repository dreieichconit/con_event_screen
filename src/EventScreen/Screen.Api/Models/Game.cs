...
public class Game
{
	...
	public string TitleString
	{
		get
		{
			...
			var brackets = new Regex(@"([\[][\w\s]+[\]])");
			var replaced = brackets.Replace(text, "");
			
			return replaced.Length > 80 ? replaced[..37]+ "..." : replaced;
		}
	}
	...
	[JsonIgnore]
	public MarkupString TableText => Tables != null ? new MarkupString(string.Join(", ", Tables.Select(x => x.Name))) : new MarkupString("");
	...
	[JsonIgnore]
	public MarkupString LabelText => GameLabels != null ? new MarkupString(string.Join(", ", GameLabels.Select(x => x.Name))) : new MarkupString("");
